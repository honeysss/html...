<?php
    ...
    if($comments == ""){
        $comments = "该用户很懒，什么都没有留下..";
    }
    $sql1 = "insert into comments values('$userName','$orderId','$time','$comments','$coState','$headImg','$starImg');";
    $sql3 = "update food set upRate = goodNum/sellNum*100;";
    $sql4 = "update orderinfo set state = 3 WHERE orderId = $orderId;";
    if(mysqli_query($con,$sql1)) {

        if (!empty($_SESSION['commentFoodName'])) {
            for ($i = 0; $i < sizeof($_SESSION['commentFoodName']); $i++) {
                $sql2 = "update food set goodNum = goodNum + $goodNum ,upNum = upNum + $upNum[$i], downNum = downNum + $badNum[$i] where foodName = '" . $_SESSION['commentFoodName'][$i] . "';";
